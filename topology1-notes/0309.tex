...
\defn. \note{Limit Point} Let \(A \subset X\). \(x \in X\) is a \textbf{limit point} of \(A\) if every neighborhood of \(x\) intersects \(A\) in some point other than \(x\). We denote the set of all limit points of \(A\) as \(A'\). i.e. for any neighborhood \(U\) of \(x\),
\[
...
\defn. \(X\) satisfies the \textbf{\(T_1\) axiom} if every one-point set is closed.\footnote{Check that Hausdorff space is \(T_1\). (\sref{Theorem 17.8})}
