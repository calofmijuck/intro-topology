...
Let \(J = \{(n, B) \in \N \times \mc{B} : B \in \mc{B}_n\}\). Define \(F: X \ra [0, 1]^J\) where \(x \mapsto \paren{f_{n, B}(x)}_{(n, B) \in J}\). If \([0, 1]^J\) is given the product topology, then we know that \(F\) is an imbedding. (\sref{Theorem 34.2}) In this cases, equip \([0, 1]^J\) with the uniform topology. It is clear that \(F\) is still injective, and that \(F\) is an open map of \(X\) onto \(Z = F(X)\), since the uniform topology is finer. But we don't know if \(F\) is continuous yet. Our task is to show that \(F\) is continuous.
