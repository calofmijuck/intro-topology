...
\begin{enumerate}
    \item \note{Regular} \(X\) is \textbf{regular} if for each pair of point \(x\) and a closed set \(B\) disjoint from \(x\), there exist disjoint open sets containing \(x\) and \(B\), respectively.
    \item \note{Normal} \(X\) is \textbf{normal} if for each pair of disjoint closed sets \(A, B\) of \(X\), there exist disjoint open sets containing \(A, B\), respectively.
\end{enumerate}
...
\thm{31.2} A subspace/product of regular space(s) is also regular.
